...
| `forge_dependency_monitor`             | Regional tenant GitHub and AWS dependency probes.                       |
...
| Splunk Observability dashboards | Metrics, dependency availability, rate-limit budget, resource pressure, runner capacity, AWS services, and OpenCost.  |
...
| 10   | `environments/prod/regions/eu-west-1/forge_dependency_monitor`    | Regional GitHub and AWS dependency probes.            |
...
| `environments/prod/regions/eu-west-1/forge_dependency_monitor/config.yml` | Regional tenant prefixes and Splunk delivery endpoints.             |
...
```bash
cd examples/deployments/integrations/terragrunt/environments/prod/regions/eu-west-1/forge_dependency_monitor
terragrunt plan
terragrunt apply
```
