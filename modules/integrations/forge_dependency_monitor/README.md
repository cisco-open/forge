...
```hcl
module "forge_dependency_monitor" {
  source = "path/to/modules/integrations/forge_dependency_monitor"
...
`splunk_o11y_conf_shared/detectors/dependency_probes`. Enable them only after
the regional probes are deployed and metric cadence is confirmed:
...
| <a name="input_name_prefix"></a> [name\_prefix](#input\_name\_prefix) | Prefix for dependency-monitor resources. | `string` | `"forge-dependency-monitor"` | no |
| <a name="input_region_alias"></a> [region\_alias](#input\_region\_alias) | Short Forge alias for the AWS region, for example usw2. | `string` | n/a | yes |
