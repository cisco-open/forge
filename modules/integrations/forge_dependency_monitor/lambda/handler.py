"""Regional Forge dependency monitor.
...
REGION_ALIAS = os.environ.get('AWS_REGION_ALIAS', '')
...
def github_request(
    ...
    *,
    ...
    headers = {
        ...
        'User-Agent': 'forge-dependency-monitor',
    }
    if api_version:
...
def queue_metrics(
    ...
    *,
    ...
    dimensions = {
        ...
        'RegionAlias': config.get('region_alias') or REGION_ALIAS,
        'Provider': provider,
    ...
    *,
    ...
    event = {
        ...
        'forgecicd_region_alias': (
            config.get('region_alias') or REGION_ALIAS
        ),
        'provider': provider,
    ...
    queued_events.append(
        {
            'source': 'forge-dependency-monitor',
...
def deliver_queued_telemetry() -> tuple[int, int, int]:
    ...
    try:
        ...
        delivery_failures += 1
        LOG.error(
            'splunk_cloud_delivery_failed error_type=%s',
            type(error).__name__,
        )
    ...
    try:
        ...
        delivery_failures += 1
        LOG.error(
            'splunk_o11y_delivery_failed error_type=%s',
            type(error).__name__,
        )
