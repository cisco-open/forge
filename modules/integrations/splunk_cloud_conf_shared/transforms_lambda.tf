...
resource "splunk_configs_conf" "forgecicd_extra_lambda_tenant_fields" {
  ...
  variables = {
    "REGEX"      = "(?<aws_region>[^:]+):\\/aws\\/lambda\\/(?<forgecicd_tenant>[a-z0-9]+)-(?<forgecicd_region_alias>[a-z0-9]+)-(?<forgecicd_vpc_alias>[a-z0-9]+)-(?<forgecicd_log_type>register-github-app-runner-group|github-webhook-relay(?:-validate-signature)?|clean-global-lock|job-log-archiver|job-log-dispatcher|forge-trust-validator(?:-prepare)?|redrive-deadletter)"
