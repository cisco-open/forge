...
resource "signalfx_list_chart" "github_availability" {
  ...
  program_text = "A = data('forge.dependency.availability', filter=(${local.metric_filter}) and filter('Provider', 'GitHub'), rollup='latest').min(by=['TenantName', 'AWSRegion', 'RegionAlias', 'CheckName']).publish(label='A')"
...
resource "signalfx_list_chart" "ssm_availability" {
  ...
  program_text = "A = data('forge.dependency.availability', filter=(${local.metric_filter}) and filter('Provider', 'AWS') and filter('CheckName', 'SSMCredentials'), rollup='latest').min(by=['TenantName', 'AWSRegion', 'RegionAlias']).publish(label='A')"
...
resource "signalfx_list_chart" "rate_limit_budget" {
  ...
  program_text = "A = data('forge.dependency.rate_limit_remaining_pct', filter=(${local.metric_filter}) and filter('Provider', 'GitHub') and filter('CheckName', 'OrgRunnersApi'), rollup='latest').min(by=['TenantName', 'AWSRegion', 'RegionAlias']).publish(label='A')"
...
resource "signalfx_time_chart" "latency" {
  ...
  program_text = "A = data('forge.dependency.latency_ms', filter=(${local.metric_filter}), rollup='average').mean(by=['TenantName', 'Provider', 'CheckName', 'RegionAlias']).publish(label='A')"
...
resource "signalfx_time_chart" "probe_execution" {
  ...
  program_text = "A = data('forge.dependency.probe_executed', filter=(${local.metric_filter}) and filter('Provider', 'Forge') and filter('CheckName', 'TenantCycle'), rollup='sum').sum(by=['TenantName', 'AWSRegion', 'RegionAlias']).publish(label='A')"
...
resource "signalfx_dashboard" "dependency_health" {
  ...
  variable {
    property               = "TenantName"
    alias                  = "Forge tenant"
    description            = "Tenant whose dependency probes should be displayed."
