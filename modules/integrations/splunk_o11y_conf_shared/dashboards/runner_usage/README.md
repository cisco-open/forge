# Splunk Observability Forge Runner Usage Dashboard

This module creates the `Forge Runner Usage` dashboard.

It tracks EC2 and Kubernetes runner adoption, active capacity, instance types,
and estimated runtime by tenant. The dashboard is separate from the
incident-focused `Forge Tenant Impact` dashboard.
...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_cluster_names"></a> [cluster\_names](#input\_cluster\_names) | Forge Kubernetes clusters included in runner usage. | `list(string)` | `[]` | no |
...
| <a name="input_tenant_names"></a> [tenant\_names](#input\_tenant\_names) | Tenant namespaces that run Forge runners. | `list(string)` | n/a | yes |
