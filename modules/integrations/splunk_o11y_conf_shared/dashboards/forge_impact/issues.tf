...
resource "signalfx_list_chart" "top_tenants_lambda_errors" {
  ...
  program_text = <<-EOF
A = data('Errors', filter=(${local.ec2_tenant_filter}) and (${var.lambda_dimension_filter}) and filter('stat', 'sum') and filter('aws_tag_ForgeModuleRef', '*'), rollup='sum', extrapolation='zero').sum(by=['aws_tag_TenantName']).sum(over=${local.issue_window}).above(0).top(count=10).publish(label='A')
EOF
...
resource "signalfx_list_chart" "top_tenants_lambda_throttles" {
  ...
  program_text = <<-EOF
A = data('Throttles', filter=(${local.ec2_tenant_filter}) and (${var.lambda_dimension_filter}) and filter('stat', 'sum') and filter('aws_tag_ForgeModuleRef', '*'), rollup='sum', extrapolation='zero').sum(by=['aws_tag_TenantName']).sum(over=${local.issue_window}).above(0).top(count=10).publish(label='A')
EOF
