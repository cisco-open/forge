...
| `Forge Metric API Ingestion Health` | Metric API problem overview, volume, drop reasons, MTS admission, and token usage | `dashboard_variables.metric_ingest` |
...
- Use `Forge Metric API Ingestion Health` when a sender reports successful
  ...
  `Timeout` is an internal post-limit signal, not a client network timeout.
