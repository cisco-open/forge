...
token secrets. The list both limits every metric-ingestion query and provides
the restricted `Token ID` dashboard selector. An empty list fails closed and
returns no organization-wide token data.
...
- Token-ingestion health uses a target-realm-validated inventory of Splunk
...
- Use `[SREA][Forge] Token ingestion health` when a sender reports successful
  ...
  signal, not a client network timeout.
