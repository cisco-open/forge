...
| `Forge Metric API Ingestion Health` | Metric API volume, drop reasons, MTS admission, token usage, and retained metric names by stable ingest source | `dashboard_variables.metric_ingest` |
...
`metric_ingest.token_ids` is a separate static allow-list of Forge-owned
Splunk Observability ingest token IDs. Token IDs are sender identifiers, not
token secrets. The list limits every token-scoped metric-ingestion query and
provides the restricted `Token ID` dashboard selector. An empty list makes
those queries fail closed and returns no organization-wide token data.

`metric_ingest.ingest_sources` restricts the separate metric-name inventory to
stable `forgecicd_ingest_source` dimension values. The chart can only show
retained MTS from senders that emit this dimension; it does not derive metric
names from token identity and cannot show rejected-before-retention data.
...
- Metric API ingestion health uses a target-realm-validated inventory of Splunk
  organization metrics with `tokenId` and `tokenName` dimensions. Populate
  `dashboard_variables.metric_ingest.token_ids` with token IDs from that realm;
  never place ingest token secret values in configuration.
- Populate `dashboard_variables.metric_ingest.ingest_sources` only after the
  matching senders emit a stable `forgecicd_ingest_source` dimension. Adding
  that dimension creates new MTS identities and must be checked against MTS
  admission limits before rollout.
...
| <a name="module_dashboard_metric_ingest"></a> [dashboard\_metric\_ingest](#module\_dashboard\_metric\_ingest) | ./dashboards/metric_ingest | n/a |
