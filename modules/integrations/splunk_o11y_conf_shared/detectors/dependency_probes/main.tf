...
lambda_errors = data('Errors', filter=filter('aws_tag_TenantName', '${each.value}') and (${var.lambda_dimension_filter}) and filter('stat', 'sum') and filter('aws_tag_ForgeModuleRef', '*'), rollup='sum', extrapolation='zero').sum(over='10m').sum(by=['aws_region', 'aws_function_name', 'aws_tag_ForgeModuleRef'])
lambda_invocations = data('Invocations', filter=filter('aws_tag_TenantName', '${each.value}') and (${var.lambda_dimension_filter}) and filter('stat', 'sum') and filter('aws_tag_ForgeModuleRef', '*'), rollup='sum', extrapolation='zero').sum(over='10m').sum(by=['aws_region', 'aws_function_name', 'aws_tag_ForgeModuleRef'])
lambda_error_rate = (lambda_errors / lambda_invocations) * 100
lambda_throttles = data('Throttles', filter=filter('aws_tag_TenantName', '${each.value}') and (${var.lambda_dimension_filter}) and filter('stat', 'sum') and filter('aws_tag_ForgeModuleRef', '*'), rollup='sum', extrapolation='zero').sum(over='5m').sum(by=['aws_region', 'aws_function_name', 'aws_tag_ForgeModuleRef'])
