...
resource "signalfx_detector" "tenant_dependency_health" {
  ...
  program_text = <<-EOF
tenant_cycle = data('forge.dependency.probe_executed', filter=filter('TenantName', '${each.value}') and filter('Provider', 'Forge') and filter('CheckName', 'TenantCycle'), rollup='latest').max(by=['AWSRegion', 'RegionAlias']).fill(value=0, duration='${var.detector_config.no_data_fill_duration}')
ssm_availability = data('forge.dependency.availability', filter=filter('TenantName', '${each.value}') and filter('Provider', 'AWS') and filter('CheckName', 'SSMCredentials'), rollup='min').min(by=['AWSRegion', 'RegionAlias']).fill(value=0, duration='${var.detector_config.no_data_fill_duration}')
github_availability = data('forge.dependency.availability', filter=filter('TenantName', '${each.value}') and filter('Provider', 'GitHub'), rollup='min').min(by=['AWSRegion', 'RegionAlias']).fill(value=0, duration='${var.detector_config.no_data_fill_duration}')
github_rate_limit_remaining_pct = data('forge.dependency.rate_limit_remaining_pct', filter=filter('TenantName', '${each.value}') and filter('Provider', 'GitHub') and filter('CheckName', 'OrgRunnersApi'), rollup='min').min(by=['AWSRegion', 'RegionAlias']).fill(value=100, duration='${var.detector_config.no_data_fill_duration}')
