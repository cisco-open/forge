# GitHub Webhook Relay Source

Internal source module for the Forge tenant GitHub webhook relay. It receives
GitHub webhook POSTs, validates the `X-Hub-Signature-256` HMAC secret, writes
accepted events to a source EventBridge bus, and forwards matching events to a
...
graph TD
  subgraph Source Account
    C[GitHub Webhook] --> H[API Gateway HTTP API]
