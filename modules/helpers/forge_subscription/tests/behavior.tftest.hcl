...
run "forge_subscription_runner_role_contract" {
  ...
  assert {
    condition = (
      ...
      && length(regexall("resources\\s*=\\s*\\[\"\\*\"\\]", file("${path.module}/policies.tf"))) == 8
    )
    error_message = "The singleton publisher policy must use wildcard resources while retaining the S3 prefix and Lambda PassRole service conditions."
  }
}
